...
int
getcolxz (double *xcol, SNP *cupt, const int *xindex, int *xtypes, int nrows,
...
void
compute_XTX (double *XTX, SNP **xsnplist, const int *xindex, double *xmean,
             double *xfancy, const int weightmode, const int nrows,
             const int ncols, int *xtypes);
void *
compute_block (void *block);
void
collect_output (double *XTX, const work_queue *work_queue, const block *blocks,
                const int num_threads);
void
partition (const int size, const int n, const int num_partitions, int *start,
           int *stop);
void
...
// -------------------------------------------------------------------------------------
      compute_XTX (XTX, xsnplist, xindex, xmean, xfancy, weightmode, nrows,
                   ncols, xtypes);
// ----------------------------------------------------------------------------------
...
domult (double *cc, const block *block)
{
  ...
  if (block->XTX_col_start == 0 && block->XTX_col_stop == block->nrows - 1
    ...
    addouterblock (block->XTX_block, cc, block->nrows, block->XTX_row_start,
                   block->XTX_row_stop, block->XTX_col_start,
                   block->XTX_col_stop);
}
             ...
             const int ncols, int *xtypes)
{
   ...
   */
  if (numthreads == THREADING_DEFAULT)
    numthreads = sysconf (_SC_NPROCESSORS_ONLN);
  if (numxtxblocksperside == THREADING_DEFAULT)
    numxtxblocksperside = nrows > 1000 ? (int) log10 (nrows * nrows) - 4 : 1;
  if (numsnppartitions == THREADING_DEFAULT)
    numsnppartitions = numthreads;

  if (numxtxblocksperside <= 0)
    fatalx ("Number of xtx blocks per side must be greater than 0.");
  if (numsnppartitions <= 0)
    fatalx ("Number of SNP partitions must be greater than 0.");
  if (numthreads <= 0)
    fatalx ("Number of threads must be greater than 0.");

#ifdef HAVE_PTHREAD
  printf (
      "Multicore implementation; using %d thread%s (%d matrix block%s per side, %d snp partition%s)\n",
      numthreads, numthreads > 1 ? "s" : "", numxtxblocksperside,
      numxtxblocksperside > 1 ? "s" : "", numsnppartitions,
      numsnppartitions > 1 ? "s" : "");
#else
  ...
  for (block_col = 0; block_col < numxtxblocksperside; block_col++)
    {
      for (block_row = 0; block_row < numxtxblocksperside; block_row++)
        {
          for (snp_partition = 0; snp_partition < numsnppartitions;
              snp_partition++)
            {
              if (thread == 0)
              ...
              partition (xtx_size, block_col, numxtxblocksperside,
                         &block->XTX_col_start, &block->XTX_col_stop);
              partition (xtx_size, block_row, numxtxblocksperside,
                         &block->XTX_row_start, &block->XTX_row_stop);
              ...
              partition (ncols, snp_partition, numsnppartitions,
                         &block->SNP_col_start, &block->SNP_col_stop);
...
void*
compute_block (void* block_holder)
{
  int i, j, k, n0, n1;
  ...
  int chrom, numclear;
  double gdis;
  double *ldmat = NULL, *ldmat2 = NULL, *ldvv = NULL, *ldvv2 = NULL,
      *vv2 = NULL;
  int xblock;
  ...
  for (i = block->SNP_col_start; i <= block->SNP_col_stop; i++)
    {
      ...
      chrom = cupt->chrom;
      getcolxz (cc, cupt, block->xindex, block->xtypes, block->nrows, i,
                block->xmean, block->xfancy, &n0, &n1);
      t = MIN(n0, n1);
      ...
      if (ldregress > 0)
        {
          ...
          if (numclear > 0)
            clearld (ldmat, ldvv, ldregress, block->nrows, numclear);
...
void
collect_output (double *XTX, const work_queue* queue, const block* blocks,
                const int num_threads)
{
  ...
  for (thread = 0; thread < num_threads; thread++)
    {
      ...
      addblocktomatrix (XTX, block->XTX_col_start, block->XTX_row_start,
                        block->nrows, block->XTX_block, block_width,
                        block_height);
           ...
           int* stop)
{
  ...
  *stop =
      (n != (num_partitions - 1)) ?
          (n + 1) * (size / num_partitions) - 1 : size - 1;
}
