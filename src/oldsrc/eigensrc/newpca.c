...
dofast(SNP **snpmarkers, Indiv **indivmarkers, int numsnps, int numindivs, int numeigs, FILE *ofile) 
{
  ...
  transpose(eigenvecs, eigenvecs, nrows, numeigs) ;
  fprintf(ofile, "%20s ", "#eigvals:") ;
/**
 this is just a dummy;  
*/

  ZALLOC(outvecs, numindivs*numsnps, double) ;
  ZALLOC(ww, numsnps, double) ;
  ZALLOC(fscal, numeigs, double) ;
  for (i=0; i<numindivs; ++i) { 
    for (j=0; j<numsnps; j++) { 
     getgval(i, j, &ww[j]) ; 
    }
    for (k=0; k<numeigs; ++k) { 
     y = outvecs[k*numindivs+i] = vdot(eigenvecs+j*nrows, ww, numsnps) ;
     if (indivmarkers[i] -> ignore == NO) fscalk] += y*y ;  // test if part of nrows
    }
  }
  vsqrt(fscal, fscal, numeigs) ;
  for (i=0; i<numindivs; ++i) { 
     outvecs[k*numindivs+i] /= fscal[k] ;
  }
